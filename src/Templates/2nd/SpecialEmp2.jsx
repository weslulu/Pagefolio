...
const EmployeeSlider = () => {
  ...
  if (loading || !designData) {
    return <div className="text-center py-20 text-gray-500 text-lg">جاري تحميل بيانات الموظفين...</div>;
  }
  ...
  const employees = sections?.eotm?.items || [];
  const title = sections?.eotm?.section_name || 'افرادنــا المتميزيــن';
  ...
  return (
    <div className="w-full px-4 py-15 flex flex-col items-center" style={{ fontFamily: fontFamily || 'inherit', backgroundColor: themeColor1 }}>
      <AnimatedSection>
      <h2 className="text-2xl sm:text-3xl font-bold mb-6 text-center" style={{ color: textColor1 }}>
        {title}
      </h2>
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        ...
        <button
          onClick={prev}
          className="w-10 h-10 sm:w-11 sm:h-11 flex items-center justify-center text-2xl sm:text-3xl rounded-full hover:text-white disabled:opacity-70"
          style={{
            color: textColor1,
            backgroundColor: `${textColor1}1a`,
          }}
          disabled={startIndex === 0}
        >
          ←
        ...
        <div className="flex gap-4 sm:gap-6 overflow-x-auto no-scrollbar">
          {visibleEmployees.map((employee) => (
            ...
            >
              <img
                src={`${employee.image}?v=${Date.now()}`}
                alt={employee.name}
                className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105"
              />

              <div
                className="absolute inset-0 bg-gradient-to-r to-transparent opacity-25 z-10"
                style={{ backgroundColor: textColor2 }}
              ></div>

              <div
                className="absolute inset-0 flex items-center justify-center font-bold z-20 text-center px-2 py-45"
              ...
              <div
                className="absolute top-0 left-0 w-8 h-8 sm:w-10 sm:h-10 opacity-75 rounded-br-full z-30"
                style={{ backgroundColor: textColor2 }}
              ></div>

              <div
                className="absolute bottom-0 right-0 w-12 h-12 sm:w-14 sm:h-13 rounded-tl-full z-30 group-hover:bg-opacity-80 transition duration-300"
                style={{ backgroundColor: themeColor1 }}
              ></div>

              <div
                className="absolute flex items-center justify-center group-hover:right-0 group-hover:rounded-b-none group-hover:bottom-0 
                group-hover:items-center group-hover:justify-center group-hover:w-full group-hover:h-full -bottom-32 -right-16 w-36 h-48 
                rounded-full transition-all duration-500 group-hover:px-6 sm:group-hover:px-8 2xl:group-hover:px-14 group-hover:pt-10
                lg:group-hover:pt-8 2xl:group-hover:pt-10 z-40 text-center"
                style={{ backgroundColor: textColor2, color: themeColor2 }}
              >
                <p className="text-justify hidden group-hover:block text-sm sm:text-base 2xl:text-lg font-semibold">
                  {employee.content}
                </p>
