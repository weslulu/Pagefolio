...
function SpecialEmploy() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await getCompanyInfo(1);
        const pageData = res.data.data[0];
        const section = pageData.sections[0];

        const employeesData = section.eotm_title.employee_of_the_months;

        const themeColor1 = pageData.theme_color1;
        const themeColor2 = pageData.theme_color2;
        const textColor1 = pageData.text_color1;
        const textColor2 = pageData.text_color2;

        const formattedEmployees = employeesData.map(emp => ({
          id: emp.id,
          name: emp.employee_name,
          description: emp.content,
          image: emp.image?.image_url || '',
        }));

        setEmployees(formattedEmployees);
        setColors({ themeColor1, themeColor2, textColor1, textColor2 });
      } catch (error) {
  ...
  return (
    <div className="w-full flex justify-center items-center py-8 sm:py-5 md:py-5 lg:py-20 px-1" style={{ backgroundColor: colors.themeColor1 }}>
      <div className="px-30 py-8 max-w md:py-5 w-full relative" style={{ backgroundColor: colors.themeColor1 }}>
        <h2 className="font-medium text-3xl p-2 mb-4 text-center" style={{ color: colors.textColor2 }}>افرادنــا المتميزيــن</h2>
        {employees.length > 0 && (
          <div className="flex flex-col lg:flex-row items-center gap-4">
            <div 
              onClick={handlePrev} 
              className="absolute h-full w-1/6 cursor-pointer z-10 transition-area left-0 flex items-center justify-start pl-4"
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 24 24" fill="none" stroke={colors.textColor1} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <path d="M15 18l-6-6 6-6"></path>
              </svg>
            </div>
            ...
            >
             <div
  className="relative flex flex-col lg:flex-row items-center xl:ml-25 p-3 md:px-6 rounded-lg"
  style={{
    background: `linear-gradient(to right, ${colors.themeColor2}, ${colors.themeColor1})`,
  }}
>

                <img src={`http://68.183.28.116/storage/${employees[current].image}`} alt={employees[current].name} 
                  className="lg:max-w md:ml-9 md:mr-9 md:mt-5 sm:mt-4 sm:ml-6 sm:mr-4 lg:ml-0 lg:mr-10 lg:mb-4 h-auto object-cover rounded-lg mr-5 ml-5 mb-0" />
                <div className="w-full text-center lg:text-right lg:w-2/3" style={{ color: colors.textColor2 }}>
                  <h3 className="text-xl py-7 lg:max-w-[500px] font-bold">{employees[current].name}</h3>
                  <p className="text-lg lg:max-w-[400px] lg:text-lg leading-relaxed">{employees[current].description}</p>
                </div>
              </div>
